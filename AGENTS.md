...
- Local model requires Ollama running at `localhost:11434`. The URL is overridable via the `CLAWSTER_OLLAMA_URL` env var (CLA-62, a dev convenience — no store setting, no Settings UI; nothing gates it on `isDev`, so a prod build without `FIREWORKS_BASE_URL` also honors it) to point the app and test harnesses at a remote/self-hosted Ollama, e.g. a Mac Mini over LAN (`CLAWSTER_OLLAMA_URL=http://<mini-ip>:11434`) or a Cloudflare Tunnel (`CLAWSTER_OLLAMA_URL=https://<name>.trycloudflare.com`). Resolved once via `resolveToolBaseUrl`/`resolveOllamaUrl` (`src/main/chat/ollama-url.ts`); unset = localhost default, so behavior is unchanged. Prod keeps Fireworks priority (`FIREWORKS_BASE_URL` beats it); `LocalToolProvider`'s own constructor default is untouched
