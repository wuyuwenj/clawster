...
- The fine-tuned local tool model emits at most ONE tool call per turn, so a multi-request like "play kpop and also what's my battery" runs only `play_music` and drops the battery ask (CLA-37). This is a model limitation, not an executor drop — `ChatRouter` classifies once and executes one tool. The response-layer mitigation lives in `src/main/chat/multi-tool.ts`: `detectSecondaryRequest(input, handledTool)` spots the dropped request and Clawster acknowledges it in the reply text ("…want me to also check your battery?") so the user can type the follow-up as its own turn — text only, no quick-reply buttons, since PetChat's buttons deliberately don't dispatch arbitrary commands. It's deliberately conservative — it only fires when the handled request AND a second, distinct-tool request appear in **separate clauses** split on a conjunction (and/also/then/comma/…), so single requests that merely name two topics ("remind me to check the weather") produce no offer. Wired into all four tool-success branches of `chat`/`chatStream`; test in `test/multi-tool.test.ts` mocks `tool-executor` so no real tools (incl. audio) run
