...
- The Animalese voice (CLA-53, v2) is **sampled playback**: `AnimaleseEngine` (`src/renderer/utils/animalese.ts`) plays one bundled clip per letter via `AudioBufferSourceNode.playbackRate` (pitch-shift). All expressive logic — sentence pitch contour, mood→pitch/speed, punctuation pauses, trail-off, and the emoji/number/non-Latin skip rules — is the **pure** `planUtterance()` in `src/renderer/utils/animalese-prosody.ts` (no audio imports, unit-tested with plain assertions in `test/animalese-prosody.test.ts`; `animalese.ts` re-exports it). `speak(text, mood?)` infers a `VoiceMood` from the text when `mood` is omitted (`inferVoiceMood`); `moodToVoice` maps the app `Mood` (`idle` deliberately maps to neutral — sleepy is reserved for the sleeping/doze states). Clips live in **`src/renderer/assets/voice/`** and are **gitignored** (private licensed assets — never commit them; do not rip AC audio); Vite bundles them via `import.meta.glob` at build. With **no clips present** (public checkout / CI / tests) the engine degrades **silently** — visemes/mouth still run, no audio — so the public build and full suite pass clip-absent. Mute still gates `.start()` (never blocks visemes/timing); the mute e2e (`e2e/mute-toggle.spec.ts`) observes this by patching `createBufferSource` in an init script (counted, never-started sources — the suite makes no sound) and seeding a silent voice bank through the `window.__clawsterVoice` hook `animalese.ts` exposes, so it passes clip-absent. The hook is dev-only (gated on `import.meta.env.DEV`, never in the packaged app), so the spec self-skips in prod mode (`E2E_MODE=prod`) — mute coverage there stays with `test/mute-toggle.test.ts`. The real voice bank and a phoneme/CMUdict upgrade are captain-gated follow-ups. The native `speech-helper` is STT (mic) only — never route TTS through it
