...
- The whole pointer gesture (movement threshold, resistance window, sub-pixel delta accumulator, drag reaction) lives in the `DragGesture` state machine in `src/renderer/pet/drag-interactions.ts`; `Pet.tsx` only renders its decisions and re-arms the timer it asks for. Its resistance window is driven by a scheduled deadline, not by pointer events, so a press that holds still still resolves
- Dragging a pet that is mid-walk sends `pet-drag-take-over` on the event the drag begins, so main calls `cancelMoveAnimation()` and stops the eased 16ms move interval from overwriting the dragged position. `animateMoveTo` / `executePetAction` therefore resolve a `MoveOutcome` (`'completed' | 'cancelled'`), which the `execute-pet-action`, `move-pet-to`, and `move-pet-to-cursor` IPC handlers surface to the renderer as `{ completed: boolean }`
- In `src/renderer/pet/styles.css` the `.state-dragging` block must stay last: it is applied alongside the `.state-*` / `.idle-*` / `.action-*` classes at equal specificity, so the carried claw pose only wins by being declared after all of them (enforced by `test/drag-interactions.test.ts`)
...
### README marketing screenshots (CLA-60)
`assets/screenshots/quick-chat.png` + `chat-popup.png` are regenerated by `npm run capture:screenshots` (`scripts/capture-marketing-screenshots.mjs`) — they are NOT hand-captured. The originals were shot off a developer's desktop with no recipe and silently went stale when Tidepool (CLA-58) landed; re-run this after any chatbar/pet-chat restyle.
- Needs the Vite dev server for THIS worktree: `npx vite --port 5199 --strictPort`, then `VITE_DEV_PORT=5199 npm run capture:screenshots`. Do **not** rely on a bare `:5173` — `playwright.config.ts` sets `reuseExistingServer`, and a Vite left running by another checkout will silently serve a different codebase into the art.
- Stage 1 captures the **real** chatbar / pet-chat / pet windows via Playwright's Electron driver; stage 2 composites them in Chromium at DPR 2. Every Clawster pixel is a real window capture — only the wallpaper and the generic (unbranded) document window behind are set dressing. Surfaces are placed **1:1** (half their DPR-2 pixel size), so the pet and chatbar keep the true relative scale a user sees; don't enlarge the pet to flatter it.
- The shots are captured in the **Light (Tidepool) theme**, while dark is the app default — the README says so under the table. `assistant.png` is still a hand-made dark capture.
- Gotchas: the chatbar renders its answer via the `speech-error` IPC (the one main→renderer channel that sets ChatBar's response state directly), so no network/model call is needed; the response panel caps at 200px, and the script throws if the answer overflows it or if the capsule isn't Tidepool cream. Scrollbars are hidden for the shot because macOS overlay scrollbars auto-hide at rest but an automated capture freezes one mid-fade.
- Frame sizes (1592×992 / 726×676) match the committed originals so the README table doesn't reflow; the pet-chat bubble is `max-w-[300px]`, so a longer tip grows it taller, not wider, and the script fails rather than squashing the art.
- The pet/pet-chat/chatbar captures pass Playwright's `animations: 'disabled'`, which cancels the infinite idle CSS keyframes (breathe/blink/bob/dart) back to their initial frame — a neutral, eyes-open pose — so the trimmed bounding box and the composite layout are deterministic run-to-run by construction rather than by luck. The committed PNGs are regenerated with this flag; re-running on an unchanged UI won't dirty the worktree.
- The Electron audio-safety flags (`--mute-audio` + the fake-media-device flags) and the `findWindow` helper are shared with the e2e suite via `e2e/electron-launch.mjs` (typed for TS by `e2e/electron-launch.d.mts`), so hardening them in one place can't silently drift out of sync between the tests and this capture run.
