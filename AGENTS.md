...
- `native/speech-helper/main.swift` captures the mic and transcribes on-device with whisper.cpp — no audio leaves the machine. It speaks the same line-delimited JSON protocol as before (`start`/`stop`/`quit` on stdin → `status`/`partial`/`final`/`error` on stdout), so the `speech-start`/`speech-stop` IPC surface and the renderer (`src/renderer/`, `src/main/preload.ts`) are unchanged. `src/main/speech.ts` did change: it gained model-readiness gating and error mapping — detecting the helper's model-load failure, turning it into an `absent`/`corrupt`/`unrecoverable` verdict, and translating that into one path-free user message
...
- `build:speech` compiles the helper for the host arch only. The macOS distribution scripts (`dist`, `dist:mac`) go through `build:universal` → `build:speech:universal`, which passes `--universal` to build both slices and `lipo` them into one binary — required because `mac.target` still ships an x64 dmg/zip. It fails loudly rather than shipping a single-arch helper. A flag rather than an env var, since npm runs scripts through `cmd.exe` on Windows and `dist` is the platform-agnostic entry point
