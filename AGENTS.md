...
- The chatbar window is **reused across summons** (toggle hides it, re-summon calls `.show()`), so the React root mounts once and mount-time CSS entrance animations only play on the first summon. Main sends `chatbar-shown` to the chatbar renderer on every show so it can replay the Light-theme `capsuleIn` entrance (CLA-59, `chatbar/capsule-entrance.ts`; dark keeps its first-mount-only fade). The send is **explicit at the show call sites** in `createChatbarWindow` — do NOT hang it off `window.on('show')`: macOS does not reliably emit BrowserWindow show/hide events, especially on fast hide→show sequences
...
- Playwright E2E needs `dist/main` built first (`npx tsc -p tsconfig.main.json`) — `package.json` `main` points at `dist/main/main.js`, and in a fresh checkout `electron.launch()` hangs forever (no error) without it. Rebuild after editing `src/main/**` or the e2e keeps running the old main/preload
- Playwright's `recordVideo` option also hangs `electron.launch()` (never resolves) — for motion evidence use a CDP `Page.startScreencast` session on the window's page and assemble frames with ffmpeg (see `e2e/cla59-capsule-entrance.spec.ts`)
