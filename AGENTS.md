...
- `build:speech` compiles the helper for the host arch only. `dist:mac` sets `SPEECH_HELPER_UNIVERSAL=1`, which builds both slices and `lipo`s them into a universal binary — required because `mac.target` still ships an x64 dmg/zip. It fails loudly rather than shipping a single-arch helper
